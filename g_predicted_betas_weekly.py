...
with open("Outputs/covariance_matrix_weekly_factors.pkl", "rb") as f:
...
def load_standardized_betas_from_key(key):
    ...
    filename = f"Betas/{method}_standardized_{window}w.csv"
...
for key, cov_dict in cov_store.items():
    ...
    for w, cov_matrix in cov_dict.items():

        w = pd.to_datetime(w)

        # Skip dates not available in betas
        if w not in standardized_betas.index:
            continue

        # cov_matrix: np.ndarray (2x2)
        f = cov_matrix[0, 1] / cov_matrix[0, 0]

        valid_betas = standardized_betas.loc[w].dropna()
        predicted_betas = 1 + f * valid_betas

        predicted_betas_dict[w] = predicted_betas
...
with open("Outputs/predicted_betas_weekly_models.pkl", "wb") as f:
...
k = 'wls_ewma_standardized_alpha_104'
