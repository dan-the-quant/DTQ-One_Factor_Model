...
with open("Outputs/covariance_matrix_monthly_factors.pkl", "rb") as f:
...
def load_standardized_betas_from_key(key):
    ...
    filename = f"Betas/{method}_standardized_{window}m.csv"
...
for key, cov_dict in cov_store.items():
    ...
    for m, cov_matrix in cov_dict.items():

        m = pd.to_datetime(m)

        # Skip dates not available in betas
        if m not in standardized_betas.index:
            continue

        # cov_matrix: np.ndarray (2x2)
        f = cov_matrix[0, 1] / cov_matrix[0, 0]

        valid_betas = standardized_betas.loc[m].dropna()
        predicted_betas = 1 + f * valid_betas

        predicted_betas_dict[m] = predicted_betas
...
with open("Outputs/predicted_betas_monthly_models.pkl", "wb") as f:
...
k = 'wls_ewma_standardized_alpha_24'
