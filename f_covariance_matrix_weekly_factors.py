...
weekly_market_sa = weekly_market.filter(like=pattern)
weekly_beta_sa   = weekly_beta.filter(like=pattern)

#%% 2. SPECIFICATIONS

specs_beta = list(weekly_beta_sa.columns)
...
for spec_beta in specs_beta:
    ...
    cov_store[spec_name] = build_covariance_from_spec(
        market_df=weekly_market_sa,
        beta_df=weekly_beta_sa,
...
with open("Outputs/covariance_matrix_weekly_factors.pkl", "wb") as f:
